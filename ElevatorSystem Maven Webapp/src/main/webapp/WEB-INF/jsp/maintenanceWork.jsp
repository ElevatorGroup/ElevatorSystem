...
<link href="${pageContext.request.contextPath }/statics/css/main.css"
	rel="stylesheet">
<link
	href="${pageContext.request.contextPath }/statics/font/iconfont.css"
	rel="stylesheet">
<link
	href="${pageContext.request.contextPath }/statics/css/bootstrap-select.css"
	rel="stylesheet">
<link rel="stylesheet"
...
<body>
	...
	<div class="header">
		...
		<h1 class="fl header-tit">孝南区质监局电梯安全管理系统</h1>
	...
	<div class="main">
		...
		<div class="vertical-menu-wr">
			<div class="slide-btn-box slide-btn-up hidden">
				<i class="iconfont icon-down-trangle"></i>
			</div>
			<div class="slide-btn-box slide-btn-down hidden">
			...
			<div class="vertical-menu-bd">
				...
				<div class="presoner">
					<a href="#"><img
						src="${pageContext.request.contextPath }/statics/images/personer.png" />
					</a>
				</div>
				<div class="vertical-menu">
					<ul>
						<li><a href=""><i class="iconfont icon-xinxi"></i><em>消息提醒</em><span
								class="layui-badge">99+</span> </a></li>
						<li><a class="curr" href=""><i
								class="iconfont icon-loupan"></i><em>楼盘管理</em> </a>
							<div class="vertical-menu-list">
								<a class="fl" href=""><i class="iconfont icon-jiuyuan"></i><em>楼盘信息修改</em>
								</a> <a class="fl" href=""><i class="iconfont icon-dianti"></i><em>添加电梯信息</em>
								</a> <a class="fl" href=""><i
									class="iconfont icon-manufacturer-new-add"></i><em>添加厂商安装</em>
								</a> <a class="fl" href=""><i class="iconfont icon-hetong1"></i><em>添加合同信息</em>
								</a>
							</div></li>
						<li><a href=""><i class="iconfont icon-wuye"></i><em>物业信息</em>
						</a></li>
						<li><a href=""><i class="iconfont icon-anzhuang"></i><em>安装公司</em>
						</a></li>
						<li><a href=""><i class="iconfont icon-dianti1"></i><em>电梯厂商</em>
						</a></li>
						<li><a href=""><i class="iconfont icon-bf-sos"></i><em>紧急救援</em>
						</a></li>
						<li><a href=""><i class="iconfont icon-hetong"></i><em>合同信息</em>
						</a>
						</li>
						<li><a href=""><i class="iconfont icon-hetong"></i><em>楼盘信息</em>
						</a>
						</li>
						<li><a href=""><i class="iconfont icon-hetong"></i><em>维保管理</em>
						</a>
							<div class="vertical-menu-list">
								<a class="fl" href=""><i class="iconfont icon-dianti"></i><em>公司信息管理</em>
								</a> <a class="fl" href=""><i class="iconfont icon-hetong1"></i><em>维保人员管理</em>
								</a> <a class="fl" href=""><i class="iconfont icon-hetong1"></i><em>维保工作管理</em>
								</a> <a class="fl" href=""><i class="iconfont icon-jiuyuan"></i><em>维保记录管理</em>
								</a> <a class="fl" href=""><i class="iconfont icon-jiuyuan"></i><em>维保合同管理</em>
								</a>
							</div></li>
		...
		<div class="main-r">
			<div class="defaultAre">
				...
				<form action="" method="post" class="elevatorSerch-fu">
					<div class="serchAre clearfix">
						<div class="selectAre">
							...
							<div class="serchAreform-group">
								...
								<select class="selectpicker" name="buildingId"
									...
									<c:forEach items="${buildingName}" var="item"
										varStatus="status">
										<option value="${item.id}"
											<c:if test="${buildingId == item.id}">selected</c:if>>
						...
						<div class="selectAre">
							...
							<div class="serchAreform-group">
								<select class="selectpicker" name="elevatorType"
									value="${elevatorType}">
									<option name="elevatorType" value="">请选择电梯类型</option>
									<c:forEach var="item" items="${elevatorTypeList}"
										varStatus="status">
										<option value="${item.valueId}"
											<c:if test="${elevatorType == item.valueId}">selected</c:if>>
						...
						<div class="form-group">
							<label for="" class=" control-label">设备识别码</label> <input
								type="text" name="elevatorCode" value="${elevatorCode }"
								class="form-control">
						</div>
						...
						<div class="form-group form-group-code">
							<label for="" class=" control-label">注册代码</label> <input
								type="text" name="registrationCode" value="${registrationCode}"
								class="form-control">
						</div>
						<!-- 7注册状态 -->
						<div class="selectAre">
							...
							<div class="serchAreform-group">
								<select class="selectpicker" name="registrationStatus"
									...
									<c:forEach items="${registrationStatusList}" var="item"
										varStatus="status">
										<option value="${item.valueId }"
											<c:if test="${registrationStatus==item.valueId }" >selected</c:if>>${item.valueName}</option>
									</c:forEach>
								</select>
						...
						<div class="selectAre">
							...
							<div class="serchAreform-group">
								<select class="selectpicker" name="usingState"
									value="${usingState}">
									<option name="usingState" value="">请选择使用状态</option>
									<c:forEach var="item" items="${usingStateList}"
										varStatus="status">
										<option value="${item.valueId}"
											<c:if test="${usingState == item.valueId}">selected</c:if>>
											${item.valueName}</option>
									</c:forEach>
									<select>
							</div>
				...
				<!--}serch-->
				<h3 class="tith3">维保人员分配</h3>
				<button class="layui-btn layui-change-checkbox" data-type="getCheckLength">分配</button>
		...
		<!--}右边-->
		<!--footer{-->
		<div class="footer">湖北省孝感市孝南区孝南区|© 2017 cdoptima Inc. All Rights
			Reserved &nbsp;&nbsp;</div>
		<!--}footer-->
	</div>
	...
	<script
		src="${pageContext.request.contextPath }/statics/js/bootstrap-select.js"></script>
	<script
		src="${pageContext.request.contextPath }/statics/layui/layui.js"
		charset="utf-8"></script>
	<script src="${pageContext.request.contextPath }/statics/js/table1.js"
		charset="gbk"></script>
	<script
	...
	<script src="${pageContext.request.contextPath }/statics/js/regular.js"
		charset="gbk"></script>
	<script
